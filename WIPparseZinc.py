...
# This counter increments up to the block size, it is reset for each new file.
ligC = 0

# This counts the files as they are written, it is never reset.
fileCount = 1

# This increments all the way through the run, it is never reset.
totC = 0

# This counter records the position of totC at the start of each output file creation.
startTotC = 1

# Used to name the putput file
startFileName = 1

# Determining if the total ligand count is less than the required bock size
# no the usual case.
if totLigs < blkSize:
   endFileName = totLigs
else:
...
destFileName = str(strFileCount) + "_" + str(startFileName)  + "_" + str(endFileName)+ ".sdf"
...
for line in source:
   ...
   if ligC == blkSize:
      if fileCount < numFiles:
         ...
         # determining name for next output file.
         startFileName = endFileName + 1
         if ((blkSize * fileCount) > totLigs):
           endFileName = totLigs
         else:
           endFileName = str(blkSize * fileCount)
         
         destFileName = str(fileCount) + "_" + str(startFileName) + "_" + str(endFileName) + ".sdf"
